...
/*
This one detects how many cameras are connected and then saves a stream of images into a folder for every camera
*/
...
void camera_threads(int fd, char cwd[PATH_MAX], int device) {

	std::cout << "Saving to " << cwd << std::endl;
...
// To record images
	...
	for (;;) {
		...
		if ( video_mode==RAW16 ) {
			...
			if (zoom_enable==0) {
                		sprintf(label, "%s %i: RAW16  Linear", thermal_sensor_name, device);
                    		imshow(label, thermal16_linear);
          		} else {
			        resize(thermal16_linear, thermal16_linear_zoom, size);
                     		sprintf(label, "%s %i: RAW16  Linear Zoom", thermal_sensor_name, device);
                     		imshow(label, thermal16_linear_zoom);
                	}


	        	if (record_enable==1) {
        	        	sprintf(filename, "%s%s_raw16_%lu.tiff", cwd, thermal_sensor_name, frame);
                		imwrite(filename, thermal16 , compression_params );
                        	sprintf(filename, "%s%s_agc_%lu.tiff", cwd, thermal_sensor_name, frame);
		...
		// DATA in YUV
			...
			if (record_enable==1) {
                        	sprintf(filename, "%s%s_yuv_%lu.tiff", cwd, thermal_sensor_name, frame);
...
int main(int argc, char** argv )
{
	...
	// multithreading for each device found
	// open threads
	// I'm about to do somehting criminal just to get it done, but since I know there will only be 6 cameras I am going to do a if block for up to 6 cameras and open a thread for each
	...
	if (devicesLength >= 1) {
		...
		sprintf(folder_name, "camera_capture_%i", devices[0]);
	    mkdir(folder_name, 0700);
		...
		th1 = std::thread(camera_threads, fd, cwd, devices[0]);

		if (devicesLength >= 2) {
			sprintf(video, "/dev/video%i", devices[1]);
			int fd = open(video, O_RDWR);
			sprintf(folder_name, "camera_capture_%i", devices[1]);
			mkdir(folder_name, 0700);
			...
			th2 = std::thread(camera_threads, fd, cwd, devices[1]);

			if (devicesLength >= 3) {
				sprintf(video, "/dev/video%i", devices[2]);
				int fd = open(video, O_RDWR);
				sprintf(folder_name, "camera_capture_%i", devices[2]);
				mkdir(folder_name, 0700);
				...
				th3 = std::thread(camera_threads, fd, cwd, devices[2]);


				if (devicesLength >= 4) {
					sprintf(video, "/dev/video%i", devices[3]);
					int fd = open(video, O_RDWR);
					sprintf(folder_name, "camera_capture_%i", devices[3]);
	   				mkdir(folder_name, 0700);
					...
					th4 = std::thread(camera_threads, fd, cwd, devices[3]);

					if (devicesLength >= 5) {
						sprintf(video, "/dev/video%i", devices[4]);
						int fd = open(video, O_RDWR);
						sprintf(folder_name, "camera_capture_%i", devices[4]);
						mkdir(folder_name, 0700);
						...
						th5 = std::thread(camera_threads, fd, cwd, devices[4]);


						if (devicesLength >= 6) {
							sprintf(video, "/dev/video%i", devices[5]);
							int fd = open(video, O_RDWR);
							sprintf(folder_name, "camera_capture_%i", devices[5]);
							mkdir(folder_name, 0700);
							...
							th6 = std::thread(camera_threads, fd, cwd, devices[5]);
